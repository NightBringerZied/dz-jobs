...
import Ha from "../candidates/component/ha";
export default function Layout({ children }) {
    let path = usePathname();
    let pathName=""
    if(path="/recruterses/profile")
      { pathName="recruter Profile"}
  ...
  <div className="flex h-screen ">
    ...
    >
        ...
        <div className="text-2xl font-bold flex justify-around items-center border-b-2 border-[#27AE60] h-[60px] ">
          ...
          {!isSidebarOpen &&
            <div>
              ...
              <p className=" text-lg text-gray-400">Recruter Dashboard </p>
            </div>}
        </div>
        <nav className="mt-4 flex flex-col items-center justify-center gap-5 w-full py-6 border-b-2 border-[#27AE60]">
                <Ha title={"Profile Mangaments"} notselectedimg={"/profile.png"} selectedimg={"/profile.png"} isopen={isSidebarOpen}/>
                <Ha title={"Job Search "} notselectedimg={"/jobsearch.png"} selectedimg={"/greenjob.png"}   isopen={isSidebarOpen}/>
                <Ha title={"Job Application "} notselectedimg={"/jobappli.png"} selectedimg={"/greenjob.png"} isopen={isSidebarOpen}/>
                <Ha title={"Favorite "} notselectedimg={"/favorite.png"} selectedimg={"/favorite.png"}  isopen={isSidebarOpen}/>
        </nav>
      </div>
      <div className="flex flex-col justify-around py-6 items-start">
          <div className="mt-4 flex flex-col items-center justify-center gap-5 w-full py-6 ">
                <Ha title={"Setting"} notselectedimg={"/setting.png"} selectedimg={"/setting.png"} isopen={isSidebarOpen}/>
                <Ha title={"FAQ"} notselectedimg={"/FAQ.png"} selectedimg={"/FAQ.png"} isopen={isSidebarOpen}/>
          </div>
    ...
    <div className="flex-1 flex flex-col ">
      <header className={`bg-white shadow-2xl ${isSidebarOpen ? "w-full pl-[80px]" : "lg:pl-[220px] w-full "} h-[60px] text-[#95a1af] p-4 flex justify-between items-center`}>
        <div className="flex flex-row justify-center items-center gap-4">
          ...
          <h1 className="text-2xl text-black font-semibold ">{pathName}</h1>
        </div>
        <h1 className="text-lg font-bold">Dashboard</h1>
      </header>
      <main className={`flex-1 ${isSidebarOpen ? "pl-[80px]" : "lg:pl-[220px]"} overflow-y-auto bg-[#d7dde2]`}>
        {children}
      </main>
    </div>
  </div>
